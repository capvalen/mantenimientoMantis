...
if(!isset($_GET['placa'])){ ?>
	...
	if($resultado->num_rows>=1){
		...
		while($row=$resultado->fetch_assoc()){
			$descripcionSaltos = nl2br($row['mantDescipcion']); ?>
			<tr>
				...
				<td class="tdFecha">
					...
					<span class="d-print-flex d-none"><?= $descripcionSaltos; ?></span>
				</td>
				<td class="tdTipoMant d-print-none" data-id="<?= $row['idTipoMantenimiento'];?>"><?= $row['tipmDescripcion']; ?></td>
				<td class="tdDescipcion"><?= $descripcionSaltos; ?></td>
				...
				<td class="d-print-none tdMonto"><?= $row['mantMonto']; ?></td>
				<td class="d-print-none tdArchivo" title="Descargar archivo"><?php if(strlen($row['mantAdjunto'] ?? '')>0){ ?>
					<a href="./files/<?= $row['mantAdjunto'];?>" download><i class="bi bi-file-arrow-down"></i></a> <?php }  ?>
				</td>
				<td class="d-print-none tdFactura" title="Descargar factura"><?php if(strlen($row['mantFactura'] ?? '')>0){ ?>
